...
export const RestaurantsService = {
  async getRestaurants(params: RestaurantQueryParams): Promise<RestaurantsListResponse> {
    const {
      search,
      type,
      city,
   ...
   */
  async enrichRestaurantsBatch(restaurants: Restaurant[]): Promise<Restaurant[]> {
    if (restaurants.length === 0) {
  ...
  async enrichRestaurantWithHappyHour(restaurant: Restaurant): Promise<Restaurant> {
    const restaurantId = restaurant.id
    ...
    const hoursByDay = this.buildHoursByDay(restaurantHours)
